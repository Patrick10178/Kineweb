...
function activarFecha(tipo) {
    // Ocultar los placeholders y mostrar los selectores correspondientes
    if(tipo === 'tipo-usuario') {
        document.getElementById("selects-tipo-usuario").style.display = "block";
        document.getElementById("tipo-usuario-placeholder").style.display = "none";
    } else if(tipo === 'sexo') {
        document.getElementById("selects-sexo").style.display = "block";
        document.getElementById("sexo-placeholder").style.display = "none";
    } else {
        // El caso por defecto para la fecha de nacimiento
...
function abrirModificarPaciente(datosPaciente) {
    // Asumimos que 'datosPaciente' tiene propiedades como 'rut', 'nombre', 'apellidoP', etc.
    ...
    document.querySelector("[name='contrasena']").value = ''; // Contraseña no debe ser pre-llenada por seguridad
    // Manejo de la foto (si es necesario pre-visualizarla, se necesitan ajustes adicionales)
    document.querySelector("[name='image']").value = ''; // Limpia el campo de imagen

    // Manejo de la fecha de nacimiento (usando dropdowns)
    ...
    // Configuración del tipo de usuario y hacerlo visible
    if (datosPaciente.id_cargo) {
        document.querySelector("[name='tipo_usuario']").value = datosPaciente.id_cargo;
        document.getElementById("selects-tipo-usuario").style.display = "block";
        document.getElementById("tipo-usuario-placeholder").style.display = "none";
    }

    // Configuración del sexo usando dropdown y hacerlo visible
    if (datosPaciente.sex) {
        document.querySelector("[name='sexo']").value = datosPaciente.sex;
        document.getElementById("selects-sexo").style.display = "block";
        document.getElementById("sexo-placeholder").style.display = "none";
    }

    // Configuración de la opción de administrador
    if (datosPaciente.admin) {
        console.log(datosPaciente.admin)
        if (datosPaciente.admin == 1) {
    ...
    document.getElementById('btn__registrarse').click(); // Asegúrate de que este es el ID correcto del botón
}
function abrirModificarCita(datosCita) {
    ...
    // Abrir el formulario
    document.getElementById('btn__registrarse1').click();
}
function abrirModificarCita1(datosCita) {
    // Cargar los datos en los campos del formulario
    var rutCompleto = datosCita.paciente_id ? (datosCita.paciente_id.toString() + '-' + calcularDV(datosCita.paciente_id.toString())) : '';
    document.getElementById('cita').value = datosCita.id_cita;
    document.getElementById('rutSearch').value = rutCompleto;
